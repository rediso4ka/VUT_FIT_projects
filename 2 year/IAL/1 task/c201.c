...
void List_Dispose( List *list ) {
    ListElementPtr to_delete = list->firstElement;
    while (to_delete != NULL) { // step by step going through the entire list
        ListElementPtr tmp = to_delete->nextElement;
        free(to_delete);
        to_delete = tmp;
    }
    list->activeElement = NULL; // state after initialization
    list->firstElement = NULL;
}
...
void List_InsertFirst( List *list, int data ) {
    ListElementPtr new = (ListElementPtr) calloc(1, sizeof(struct ListElement));
    if (new == NULL) {
        List_Error();
    }
    new->data = data;
...
void List_DeleteFirst( List *list ) {
    if (list->firstElement != NULL) {
        ...
        ListElementPtr to_delete = list->firstElement;
        list->firstElement = to_delete->nextElement;
        free(to_delete);
    }
}
...
void List_DeleteAfter( List *list ) {
    if (list->activeElement != NULL) {
        if (list->activeElement->nextElement != NULL) {     // double if for safety (if there is no fast calculation)
            ListElementPtr to_delete = list->activeElement->nextElement;
