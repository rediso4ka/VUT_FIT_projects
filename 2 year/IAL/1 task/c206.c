...
void DLL_Dispose( DLList *list ) {
    DLLElementPtr to_delete = list->firstElement;
    while (to_delete != NULL) { // step by step going through the entire list
        DLLElementPtr tmp = to_delete->nextElement;
        free(to_delete);
        to_delete = tmp;
    }
...
void DLL_InsertFirst( DLList *list, int data ) {
    DLLElementPtr new = (DLLElementPtr) calloc(1, sizeof(struct DLLElement));
    if (new == NULL) {
        DLL_Error();
    }
    new->data = data;
...
void DLL_DeleteFirst( DLList *list ) {
    if (list->firstElement != NULL) {
        DLLElementPtr to_delete = list->firstElement;
        if (list->activeElement == to_delete) {
            list->activeElement = NULL;
        }
...
void DLL_DeleteLast( DLList *list ) {
    if (list->lastElement != NULL) {
        DLLElementPtr to_delete = list->lastElement;
        if (list->activeElement == to_delete) {
            list->activeElement = NULL;
        }
        if (list->lastElement == list->firstElement) {
            ...
            list->lastElement = to_delete->previousElement;
...
void DLL_DeleteAfter( DLList *list ) {
    if (list->activeElement != NULL) {                      // double if for safety
